...
import ReceiptCreate from "./components/Receipt/ReceiptCreate";
import ReceiptEdit from "./components/Receipt/ReceiptEdit";
...
function App() {
  return (
    <Admin authProvider={authProvider} dashboard={Dashboard} title="ePharmacy Delivery System - Admin Panel" dataProvider={Object.assign(jsonHalRestProvider("https://epharmacy-sa.herokuapp.com/api", httpClient))}>
    ...
    <Resource name="orders" icon={OrderIcon} options={{ label: 'Orders' }} list={OrderList} edit={OrderEdit}/>
    <Resource name="payments" icon={PaymentIcon} options={{ label: 'Payments' }} list={PaymentList} edit={PaymentEdit}/>
    <Resource name="receipts" icon={ReceiptIcon} options={{ label: 'Receipts' }} list={ReceiptList} create={ReceiptCreate} edit={ReceiptEdit}/>
